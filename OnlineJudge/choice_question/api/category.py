...
from ..serializers import CategorySerializer, CategoryCreateSerializer, CategoryTreeSerializer
...
class CategoryAPI(APIView):
    ...
    def get(self, request):
        ...
        if flat:
            ...
            return self.success(CategorySerializer(categories, many=True).data)
        else:
            # 树形结构
            root_categories = queryset.filter(parent=None).order_by('order', 'name')
            return self.success(CategoryTreeSerializer(root_categories, many=True).data)
    
    @super_admin_required
    @validate_serializer(CategoryCreateSerializer)
