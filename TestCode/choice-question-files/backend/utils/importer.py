...
class QuestionImporter:
    ...
    def import_from_json(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, question in enumerate(questions_data):
                converted_question = self._convert_question_format(question, i + 1)
    ...
    def _convert_question_format(self, question_data: Dict[str, Any], row_num: int) -> Dict[str, Any]:
        """
        转换用户JSON格式到系统内部格式
        
        Args:
            question_data: 用户提供的题目数据
            row_num: 行号（用于错误提示）
        ...
        if 'type' in question_data:
            ...
            if question_type == 'single':
                ...
                converted['question_type'] = 'multiple'
            else:
                converted['question_type'] = 'single'  # 默认单选
        else:
            converted['question_type'] = 'single'  # 默认单选
        ...
        if 'correct' in question_data:
            ...
            if isinstance(correct_answer, str):
                ...
                if correct_answer.upper() in ['A', 'B', 'C', 'D', 'E', 'F']:
                    ...
                    converted['answer'] = [answer_index] if converted['question_type'] == 'multiple' else answer_index
                else:
                    # 如果是数字字符串，直接转换
                    try:
                        ...
                        converted['answer'] = 0  # 默认第一个选项
            elif isinstance(correct_answer, (int, list)):
                converted['answer'] = correct_answer
            else:
                converted['answer'] = 0  # 默认第一个选项
        
        # 转换解析
        if 'explanation' in question_data:
            converted['explanation'] = question_data['explanation']
        
        # 设置默认值
        converted.setdefault('difficulty', 'medium')
        converted.setdefault('score', 1)
        converted.setdefault('is_public', True)
