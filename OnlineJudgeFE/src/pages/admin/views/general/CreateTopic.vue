<template>
  <div class="app-container">
    ...
    <div class="page-title">
      <h2>创建专题</h2>
      <p class="subtitle">选择一级分类作为专题，二级分类下的试卷将自动关联</p>
    </div>
    ...
    <el-form ref="topicForm" :model="topicForm" :rules="rules" label-width="120px" class="topic-form">
      ...
      <div class="form-actions">
        ...
        <el-button type="primary" @click="saveTopic" :loading="saving" :disabled="!topicForm.category_id">创建专题</el-button>
      </div>
    </el-form>
  ...
  methods: {
    ...
    async saveTopic () {
      ...
      this.$refs.topicForm.validate(async (valid) => {
        ...
        try {
          // 调试信息：检查表单数据
          console.log('创建专题 - 表单数据:', this.topicForm)
          ...
          const res = await api.createTopic(topicData)
          this.$message.success('专题创建成功')
          this.$router.push('/topic/management')
        } catch (err) {
          this.$message.error('创建失败: ' + (err.response && err.response.data && err.response.data.error || err.message))
        } finally {
          this.saving = false
        }
      })
